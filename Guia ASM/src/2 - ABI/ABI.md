...
alternate_sum_4:
  ...
  ret
```
Noten cómo la solución utiliza los registros del tamaño adecuado. Bien podría haber utilizado RDI, RSI, RDX y RCX y que funcione bien, pero nada garantiza que los 32 bits más significativos del registro vayan a estar limpios cuando el tipo de dato especificado es más pequeño. El problema se vuelve aún más molesto cuando tenemos en cuenta que los movimientos entre registros menores a 32 bits (`MOV AX, BX`) no limpian los bits más altos que corresponden al registro completo, cosa que sí garantizan los movimientos entre registros de 32 bits (`MOV EAX, EDI`), razón por la cuál quien invoque `alternate_sum_4` podrá leer el resultado en RAX sin problemas. En conclusión, siempre presten atención al tamaño del dato, usen los registros del tamaño correcto, y no van a tener problemas.
